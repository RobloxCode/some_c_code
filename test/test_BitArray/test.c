...
int test_BitArr(void) {
    ...
    if (u8_bit_arr_print(&bitArr) != 0) {
        return EXIT_FAILURE;
    }

    if (u8_bit_arr_set(&bitArr, 0) != 0) {
        return EXIT_FAILURE;
    }

    if (u8_bit_arr_set(&bitArr, 1) != 0) {
        return EXIT_FAILURE;
    }

    if (u8_bit_arr_set(&bitArr, 7) != 0) {
        return EXIT_FAILURE;
    }
    ...
    if (u8_bit_arr_clear(&bitArr, 0) != 0) {
        return EXIT_FAILURE;
    }
    ...
    int bit = 0;
    for (int i = 0; i < 8; ++i) {
        if (u8_bit_arr_get(&bitArr, i, &bit) != 0) {
            return EXIT_FAILURE;
        }
