...
void Permissions_print(struct Permissions p) {
    if (p.owner_r & PERM_DISPLAY_OWN_R) {
        printf("owner read\n");
    }
    if (p.owner_w & PERM_DISPLAY_OWN_W) {
        printf("owner write\n");
    }
    if (p.owner_x & PERM_DISPLAY_OWN_X) {
        printf("owner execute\n");
    }

    if (p.group_r & PERM_DISPLAY_GROUP_R) {
        printf("group read\n");
    }
    if (p.group_w & PERM_DISPLAY_GROUP_W) {
        printf("group write\n");
    }
    if (p.group_x & PERM_DISPLAY_GROUP_X) {
        printf("group execute\n");
    }

    if (p.other_r & PERM_DISPLAY_OTHER_R) {
        printf("other read\n");
    }
    if (p.other_w & PERM_DISPLAY_OTHER_W) {
        printf("other read\n");
    }
    if (p.other_x & PERM_DISPLAY_OTHER_X) {
        printf("other read\n");
    }
}
