...
int vec_raw_shrink(void **data, size_t *cap,
...
#define vec_at(v, i) \
    (*((__typeof__((v)->data[0]) *) \
        vec_raw_at((v)->data, (v)->size, sizeof((v)->data[0]), (i))))
...
#define vec_remove(v, i) \
    vec_raw_remove((void **)&(v)->data, &(v)->size, \
                   sizeof((v)->data[0]), (i))
...
#define vec_reserve(v, n) \
    vec_raw_reserve((void **)&(v)->data, &(v)->cap, \
                    (n), sizeof((v)->data[0]))

/* Release excess capacity — compact to current size. */
#define vec_shrink(v) \
    vec_raw_shrink((void **)&(v)->data, &(v)->cap, \
